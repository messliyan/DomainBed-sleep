...
class EEGDataset(MultipleDomainDataset):
    """基础脑电数据集类，处理sleep-edf-20、sleep-edf-78、shhs"""
    N_STEPS = 10000  # 脑电数据训练步数
    CHECKPOINT_FREQ = 500
    N_WORKERS = 4
    INPUT_SHAPE = (1, 3000)  # 单通道，3000数据点
    num_classes = 5  # 睡眠阶段分类（W, N1, N2, N3, REM）

    def __init__(self, root, test_envs, hparams, dataset_type):
        super().__init__()
        self.dataset_type = dataset_type  # 'sleepedf20', 'sleepedf78', 'shhs'
        self.root = os.path.join(root, dataset_type)
        ...
        for env_idx in range(len(self.ENVIRONMENTS)):
            env_subjects = self.environments[env_idx]
            env_data, env_labels = self._concat_subjects(all_subjects, env_subjects)
            self.datasets.append(TensorDataset(
                torch.FloatTensor(env_data).unsqueeze(1),  # 添加通道维度
                torch.LongTensor(env_labels)
            ))

    def _get_environments(self):
        """按数据集类型定义环境划分（例如按记录设备/中心划分）"""
        if self.dataset_type == 'sleepedf20':
            self.ENVIRONMENTS = ['sleepedf20_env0', 'sleepedf20_env1']
            return self._split_subjects(2)  # 分成2个环境
        elif self.dataset_type == 'sleepedf78':
            self.ENVIRONMENTS = ['sleepedf78_env0', 'sleepedf78_env1', 'sleepedf78_env2']
            return self._split_subjects(3)  # 分成3个环境
        else:  # shhs
            self.ENVIRONMENTS = ['shhs_env0', 'shhs_env1']
            return self._split_subjects(2)

    def _load_subjects(self):
        """加载所有.npz文件，返回{subject_id: (x, y)}"""
        subjects = {}
        for filename in os.listdir(self.root):
            if filename.endswith('.npz'):
                subj_id = filename.split('.')[0]
                data = np.load(os.path.join(self.root, filename))
                subjects[subj_id] = (data['x'], data['y'])
        return subjects

    def _split_subjects(self, num_envs):
        """将被试均匀分配到不同环境"""
        subjects = list(self._load_subjects().keys())
        np.random.shuffle(subjects)
        return np.array_split(subjects, num_envs)

    def _concat_subjects(self, all_subjects, subject_ids):
        """合并多个被试的数据"""
        all_x = []
        all_y = []
        for subj_id in subject_ids:
            x, y = all_subjects[subj_id]
            all_x.append(x)
            all_y.append(y)
...
# 具体数据集类
class SleepEDF20(EEGDataset):
    def __init__(self, root, test_envs, hparams):
        super().__init__(root, test_envs, hparams, 'sleepedf20')

class SleepEDF78(EEGDataset):
    def __init__(self, root, test_envs, hparams):
        super().__init__(root, test_envs, hparams, 'sleepedf78')

class SHHS(EEGDataset):
    def __init__(self, root, test_envs, hparams):
        super().__init__(root, test_envs, hparams, 'shhs')


# 在DATASETS列表添加新数据集
DATASETS.extend(['SleepEDF20', 'SleepEDF78', 'SHHS'])