...
if __name__ == "__main__":
    ...
    # 打印目标域环境名称
    print(f'目标域 (target_env): {[env_names[env_i] for env_i in args.target_env]}')
    # 打印源域环境名称
    source_envs = [i for i in range(len(dataset)) if i not in args.target_env]
    print(f'源域 (source_envs): {[env_names[env_i] for env_i in source_envs]}')
    ...
    # 初始化算法模型，参数包括输入形状、类别数、域数（源域数）和超参数
    algorithm = algorithm_class(dataset.input_shape, dataset.num_classes, len(dataset) - len(args.target_env), hparams)
